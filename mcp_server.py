...
async def get_bonding_curve_progress(
    token_address: str, ctx: Context = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        status = (
            ...
            if mock_progress < 50
                ...
                "active"
                if mock_progress < 90
                else "approaching_graduation" if mock_progress < 95 else "graduated"
            )
        )
...
async def get_latest_trades(
    token_address: str, limit: int = 10, ctx: Context = None
) -> Dict[str, Any]:
    """
    ...
    result = await bitquery.execute_query(
        query, {"tokenAddress": token_address, "limit": limit}
    )
    ...
    if "error" in result:
        return {"token_address": token_address, "trades": [], "error": result["error"]}
    ...
    try:
        ...
        for trade in trades_data:
            ...
            pair_info = {
                ...
                "buy_token": trade_info.get("Buy", {})
                .get("Currency", {})
                .get("Symbol", ""),
                "sell_token": trade_info.get("Sell", {})
                .get("Currency", {})
                .get("Symbol", ""),
            }
